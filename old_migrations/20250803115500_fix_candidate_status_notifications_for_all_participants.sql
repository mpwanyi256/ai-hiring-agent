-- Migration: Fix candidate status notifications to notify ALL job participants
-- The current create_candidate_status_notification function only notifies the job creator
-- We need to update it to use create_notification_for_job_participants instead
...
-- Ensure the trigger exists for candidate status changes
...
COMMENT ON FUNCTION create_candidate_status_notification IS 'Trigger function that creates notifications for ALL job participants when candidate status changes - updated to use create_notification_for_job_participants';

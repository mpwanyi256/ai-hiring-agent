-- Clarify candidate_details view: candidates.id is the unique candidate application identifier
-- candidate_info_id is for personal info only; profile_id is for job owner filtering
-- This migration ensures all lookups, joins, and filters use candidates.id as the candidate identifier
...
SELECT 
  c.id, -- Always use this as the unique candidate application identifier
  c.job_id,
  c.interview_token,
  ci.email,
  ci.first_name,
...
-- 3. Recreate all related functions with clear comments
...
SECURITY DEFINER
AS $$
  -- Use id (candidates.id) as the unique candidate identifier
...
DECLARE
  ...
  result JSON;
BEGIN
  -- Get candidate with candidate info using candidates.id
  SELECT 
    c.*,
    ci.first_name,
    ci.last_name,
...
COMMENT ON FUNCTION get_job_candidate_details IS 'Get paginated candidate details for a specific job with optional filtering. Always use id (candidates.id) as the unique candidate identifier.';
COMMENT ON FUNCTION get_job_candidate_stats IS 'Get statistical summary of candidates for a job.';
COMMENT ON FUNCTION get_candidate_with_info IS 'Get candidate details including candidate info using candidates.id.';
COMMENT ON FUNCTION get_candidate_details IS 'Gets candidate details including candidate info by interview token using candidates.id.';

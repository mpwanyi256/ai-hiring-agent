-- Migration: Fix question_id join issue in analytics functions
-- The responses table has question_id as TEXT, not UUID, so we can't join with job_questions.id

-- Fix the calculate_candidate_analytics function to use reasonable time ranges instead of expected_duration
...
BEGIN
    ...
    -- Since we can't join with job_questions due to question_id being TEXT, use reasonable time ranges
    SELECT 
        COALESCE(AVG(
            CASE 
...
-- Fix the calculate_response_analytics function to use reasonable time ranges

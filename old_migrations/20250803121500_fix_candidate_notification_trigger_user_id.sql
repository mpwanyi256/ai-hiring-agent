-- Migration: Fix candidate notification trigger - remove user_id reference
-- The candidates table doesn't have a user_id column, so we need to fix the trigger function

-- Drop the existing trigger first
DROP TRIGGER IF EXISTS candidate_status_notification_trigger ON candidates;

-- Update the candidate status notification function to not reference user_id
...
BEGIN
    ...
    -- Create notifications for ALL job participants (no exclusions since candidates don't have user_id)
    -- The actor exclusion will be handled by the application layer when the status is updated
    PERFORM create_notification_for_job_participants(
        ...
        -- No p_exclude_user_id - notify ALL participants since we can't determine the actor
    );
...
COMMENT ON FUNCTION create_candidate_status_notification IS 'Trigger function that creates notifications for ALL job participants when candidate status changes - no actor exclusion since candidates table has no user_id';
COMMENT ON TRIGGER candidate_status_notification_trigger ON candidates IS 'Trigger that fires when candidate status is updated to notify all job participants'; 
-- Add roles and fix RLS issues
-- This migration adds user roles and fixes remaining authentication issues
...
-- Ensure the generate_company_slug function exists and is properly accessible
-- Drop and recreate to ensure it's in the right schema with proper permissions
DROP FUNCTION IF EXISTS public.generate_company_slug(TEXT) CASCADE;
...
-- Fix RLS policies to properly allow inserts during trigger execution
-- Drop all existing conflicting policies
DROP POLICY IF EXISTS "Enable profile access" ON public.profiles;
DROP POLICY IF EXISTS "Enable company access" ON public.companies;
DROP POLICY IF EXISTS "Enable user subscription access" ON public.user_subscriptions;

-- Create new comprehensive RLS policies
-- Profiles: Allow users to see their own profile, and allow service role to insert/update
...
-- Companies: Allow users to see their company, allow service role to insert/update
...
-- User subscriptions: Allow users to see their subscriptions, allow service role to manage
...
-- Update the user_details view to include the new role field
...
-- Also grant necessary permissions for the trigger to work

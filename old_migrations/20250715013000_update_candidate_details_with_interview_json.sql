-- Update candidate_details view to include interview_details as a JSON object
...
-- 2. Drop and recreate the candidate_details view with interview_details
...
-- 3. Recreate all related functions to include interview_details
...
COMMENT ON VIEW candidate_details IS 'Comprehensive view of candidates with job, evaluation, resume, and interview data. Uses candidates.id as the unique candidate identifier (now matches candidates_info.id). interview_details is a JSON object with the interview for this candidate.';
COMMENT ON FUNCTION get_job_candidate_details(UUID, UUID, TEXT, TEXT, INTEGER, INTEGER) IS 'Get paginated candidate details for a specific job with optional filtering. Uses candidates.id as the unique candidate identifier (now matches candidates_info.id).';
COMMENT ON FUNCTION get_job_candidate_stats(UUID, UUID) IS 'Get statistical summary of candidates for a job.';
COMMENT ON FUNCTION get_candidate_with_info(UUID) IS 'Get candidate details using candidates.id (now matches candidates_info.id).';
COMMENT ON FUNCTION get_candidate_details(TEXT) IS 'Gets candidate details by interview token using candidates.id (now matches candidates_info.id).';

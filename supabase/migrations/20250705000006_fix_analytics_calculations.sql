-- Migration: Fix analytics calculations and handle missing data gracefully
-- This migration fixes the analytics calculations and improves error handling

-- Fix the get_candidate_analytics function to handle missing data and return proper structure
...
BEGIN
    ...
    -- Get comparative data with proper structure (not nested)
    SELECT 
        json_build_object(
    ...
    IF comparative_data_record IS NULL THEN
        comparative_data_record := json_build_object(
            'total_candidates', 0,
            'average_score', 0,
            'top_percentile', 0,
            'median_score', 0,
    ...
    RETURN QUERY SELECT
        ...
        comparative_data_record as comparative_data;
END;
$$ LANGUAGE plpgsql;

-- Fix the calculate_candidate_analytics function to handle AI scores properly
...
-- Fix the calculate_response_analytics function to use positive sentiment scores

-- 00160_fix_candidate_details_view_add_candidate_status.sql
-- Fix candidate_details view to include the actual candidate status from candidates table
...
SELECT 
  ...
  e.resume_summary,
  e.evaluation_type,
  -- Status derived from completion and evaluation (legacy field)
  CASE 
    ...
    ELSE 'pending'
  END as status,
  -- NEW: Actual candidate status from candidates table

-- Migration: Add ai_assessment to get_candidate_analytics function
-- This adds the AI evaluation (if any) to the analytics API response

-- Drop the old function first to allow changing the return type
DROP FUNCTION IF EXISTS get_candidate_analytics(UUID, UUID) CASCADE;
...
BEGIN
    ...
    -- Get AI assessment if it exists
    SELECT row_to_json(ae.*) INTO ai_assessment_json
    FROM ai_evaluations ae
    WHERE ae.candidate_id = candidate_uuid AND ae.job_id = job_uuid
    ORDER BY ae.created_at DESC
    LIMIT 1;
    
    -- If no AI assessment, set to null
    IF ai_assessment_json IS NULL THEN
        ai_assessment_json := NULL;
    END IF;

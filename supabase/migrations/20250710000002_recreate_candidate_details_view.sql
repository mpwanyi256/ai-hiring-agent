-- Recreate candidate_details view and related functions after structural changes
-- This migration runs after fixing candidate_id to match candidate_info_id
-- The view now uses the consistent candidate ID structure

-- 1. Create the candidate_details view with updated structure
...
-- 2. Recreate all related functions
...
COMMENT ON VIEW candidate_details IS 'Comprehensive view of candidates with job, evaluation, and resume data. Uses candidates.id as the unique candidate identifier (now matches candidates_info.id). candidate_info_id is included for backward compatibility but equals id.';
COMMENT ON FUNCTION get_job_candidate_details IS 'Get paginated candidate details for a specific job with optional filtering. Uses candidates.id as the unique candidate identifier (now matches candidates_info.id).';
COMMENT ON FUNCTION get_job_candidate_stats IS 'Get statistical summary of candidates for a job.';
COMMENT ON FUNCTION get_candidate_with_info IS 'Get candidate details using candidates.id (now matches candidates_info.id).';
COMMENT ON FUNCTION get_candidate_details IS 'Gets candidate details by interview token using candidates.id (now matches candidates_info.id).';

-- 5. Example test query (for documentation)
-- SELECT * FROM candidate_details WHERE id = '<candidate_id>' AND job_id = '<job_id>'; 
-- Force Fix Candidate Details View Duplicates Migration
-- This migration fixes the candidate_details view to eliminate duplicates without using DISTINCT on JSON fields

-- ============================================================================
-- PART 1: Force drop and recreate the view without problematic DISTINCT
-- ============================================================================

-- Force drop all dependencies
DROP FUNCTION IF EXISTS get_job_candidate_details(uuid, text, text, integer, integer) CASCADE;
DROP FUNCTION IF EXISTS get_job_candidate_stats(uuid, uuid) CASCADE;
DROP VIEW IF EXISTS candidate_details CASCADE;

-- Recreate the view with proper deduplication strategy
...
SELECT 
  ...
  -- Response count (use subquery to avoid duplicates)
  (
    SELECT COUNT(*) 
    FROM responses r 
    WHERE r.candidate_id = c.id
  ) as response_count,
  -- Evaluation data (latest evaluation only)
  ...
  e.resume_summary,
  e.evaluation_type,
  -- Resume info (latest resume only)
  cr.id as resume_id,
...
LEFT JOIN LATERAL (
  -- Get the latest evaluation for this candidate (this ensures only one evaluation per candidate)
...
LEFT JOIN LATERAL (
  -- Get the latest resume for this candidate and job
...
-- ============================================================================
-- PART 2: Recreate functions with same signatures as before
-- ============================================================================
...
BEGIN
    ...
    -- Test the specific problematic candidate
    SELECT COUNT(*) INTO test_candidate_count
    FROM candidate_details 
    WHERE candidate_info_id = '0ec005f8-5811-486d-a61a-f439ebb2c073'
      AND job_id = 'ea1301a0-c2fa-44c6-879a-38b4847e405f';
    
    RAISE NOTICE '✅ Force Fix Candidate Details Duplicates Applied';
    ...
    RAISE NOTICE '  - Total duplicate records in view: %', duplicate_count;
    RAISE NOTICE '  - Test candidate record count: %', test_candidate_count;
    
    IF view_exists AND function_exists AND stats_function_exists AND duplicate_count = 0 AND test_candidate_count = 1 THEN
        RAISE NOTICE '  - ✅ All fixes applied successfully - LATERAL joins eliminate duplicates!';
    ELSE

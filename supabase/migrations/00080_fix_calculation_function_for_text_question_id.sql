-- Migration: Fix calculation function for TEXT question_id
-- This fixes the type mismatch between responses.question_id (TEXT) and job_questions.id (UUID)
...
BEGIN
    ...
    -- Calculate simple truthiness score based on response time only (no join needed)
    SELECT 
        COALESCE(AVG(
            CASE 
...
-- Fix the calculate_response_analytics function to avoid the JOIN issue
...
BEGIN
    ...
    -- Calculate truthiness score based on response time (no expected_duration needed)
    truthiness_score := CASE 
        WHEN response_time_seconds <= 0 THEN 0
        WHEN response_time_seconds < 5 THEN 0  -- Too fast
        WHEN response_time_seconds < 15 THEN 25  -- Very fast
        WHEN response_time_seconds < 30 THEN 50  -- Fast
        WHEN response_time_seconds <= 300 THEN 100  -- Good range
        WHEN response_time_seconds <= 600 THEN 75  -- Slow but acceptable
        WHEN response_time_seconds <= 900 THEN 50  -- Very slow
        ELSE 25  -- Extremely slow
    END;

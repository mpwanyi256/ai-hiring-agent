-- Migration: Fix analytics function security context
-- The calculate_candidate_analytics function needs to run with elevated privileges to write to candidate_analytics

-- Drop the existing function and recreate it with SECURITY DEFINER
DROP FUNCTION IF EXISTS calculate_candidate_analytics(UUID, UUID);
...
BEGIN
    ...
    SELECT 
        CASE 
            WHEN avg_response_time > 0 AND avg_response_time < 30 THEN 25 -- Very fast, suspicious
            WHEN avg_response_time >= 30 AND avg_response_time < 120 THEN 75 -- Good range
            WHEN avg_response_time >= 120 AND avg_response_time < 300 THEN 90 -- Thoughtful
            WHEN avg_response_time >= 300 THEN 60 -- Very slow, might be distracted
            ELSE 50 -- Default
        END
...
GRANT EXECUTE ON FUNCTION calculate_candidate_analytics(UUID, UUID) TO anon;
GRANT EXECUTE ON FUNCTION calculate_candidate_analytics(UUID, UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION calculate_candidate_analytics(UUID, UUID) TO authenticator;
GRANT EXECUTE ON FUNCTION calculate_candidate_analytics(UUID, UUID) TO service_role;

-- Also fix the calculate_response_analytics function
DROP FUNCTION IF EXISTS calculate_response_analytics(UUID);
...
BEGIN
    ...
    -- Calculate truthiness score based on response time
    truthiness_score := CASE 
        WHEN response_time_seconds > 0 AND response_time_seconds < 30 THEN 25 -- Very fast, suspicious
        WHEN response_time_seconds >= 30 AND response_time_seconds < 120 THEN 75 -- Good range
        WHEN response_time_seconds >= 120 AND response_time_seconds < 300 THEN 90 -- Thoughtful
        WHEN response_time_seconds >= 300 THEN 60 -- Very slow, might be distracted
        ELSE 50 -- Default
    END;
    
    -- Scale truthiness to sentiment (-1 to 1 range)
...
-- Grant execute permission to all relevant roles
GRANT EXECUTE ON FUNCTION calculate_response_analytics(UUID) TO anon;
GRANT EXECUTE ON FUNCTION calculate_response_analytics(UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION calculate_response_analytics(UUID) TO authenticator;
GRANT EXECUTE ON FUNCTION calculate_response_analytics(UUID) TO service_role;

-- Add helpful comments
COMMENT ON FUNCTION calculate_candidate_analytics(UUID, UUID) IS 'Calculates and upserts candidate analytics. Runs with SECURITY DEFINER to bypass RLS.';
COMMENT ON FUNCTION calculate_response_analytics(UUID) IS 'Calculates and upserts response analytics. Runs with SECURITY DEFINER to bypass RLS.'; 
...
// Memoized selectors
export const selectActiveJobs = createSelector([selectJobsList], (jobs) =>
  jobs.filter((job) => job.isActive),
);

export const selectInactiveJobs = createSelector([selectJobsList], (jobs) =>
  jobs.filter((job) => !job.isActive),
);

export const selectJobsByFormat = createSelector([selectJobsList], (jobs) => ({
  text: jobs.filter((job) => job.interviewFormat === 'text'),
  video: jobs.filter((job) => job.interviewFormat === 'video'),
}));

export const selectJobsWithCandidates = createSelector([selectJobsList], (jobs) =>
  jobs.filter((job) => (job.candidateCount || 0) > 0),
);

export const selectJobsWithoutCandidates = createSelector([selectJobsList], (jobs) =>
  jobs.filter((job) => (job.candidateCount || 0) === 0),
);
...
export const selectJobsStats = createSelector([selectJobsList], (jobs) => {
  ...
  return {
    ...
    averageCandidatesPerJob:
      jobs.length > 0 ? Math.round((totalCandidates / jobs.length) * 10) / 10 : 0,
  };
});

export const selectRecentJobs = createSelector([selectJobsList], (jobs) => {
  return [...jobs]
    .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())
    .slice(0, 5);
});

export const selectMostPopularJobs = createSelector([selectJobsList], (jobs) => {
  return [...jobs].sort((a, b) => (b.candidateCount || 0) - (a.candidateCount || 0)).slice(0, 5);
});
...
export const selectJobsByStatus = createSelector([selectJobsList], (jobs) => ({
...
export const selectJobStatusStats = createSelector([selectJobsList], (jobs) => {
...
export const selectJobsWithQuestions = createSelector([selectJobsList], (jobs) =>
  jobs.filter((job) => {
...
export const selectJobsNeedingQuestions = createSelector([selectJobsList], (jobs) =>
  jobs.filter((job) => job.status === 'draft'),
);

// Enhanced job analytics
export const selectJobAnalytics = createSelector([selectJobsList], (jobs) => {
  const totalCandidates = jobs.reduce((sum, job) => sum + (job.candidateCount || 0), 0);
  const activeJobs = jobs.filter((job) => job.isActive);
  const jobsWithCandidates = jobs.filter((job) => (job.candidateCount || 0) > 0);

  // Jobs by format
  const formatStats = jobs.reduce(
    (acc, job) => {
  ...
  return {
    ...
    conversionRate:
      activeJobs.length > 0 ? Math.round((jobsWithCandidates.length / activeJobs.length) * 100) : 0,
  };
});

// Performance selectors
export const selectTopPerformingJobs = createSelector([selectJobsList], (jobs) => {
  return [...jobs]
...
export const selectRecentlyUpdatedJobs = createSelector([selectJobsList], (jobs) => {
  return [...jobs]
...
export const selectJobsCanStartInterviewing = createSelector([selectJobsList], (jobs) =>
  jobs.filter((job) => job.status === 'draft' && job.isActive),
);

export const selectJobsCanBeClosed = createSelector([selectJobsList], (jobs) =>

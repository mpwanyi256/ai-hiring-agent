...
export const selectUsageCounts = createSelector(
  [selectUser],
  (user) => {
    if (!user) return null;
...
export const selectUsageLimits = createSelector(
  [selectUser],
  (user) => {
    if (!user || !user.subscription) return null;
...
export const selectCanCreateJob = createSelector(
  [selectUsageLimits],
  (limits) => {
...
export const selectCanConductInterview = createSelector(
  [selectUsageLimits],
  (limits) => {
    if (!limits) return false;
